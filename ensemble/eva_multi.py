...
def build_inst_prompt(task, model, question, llms_config, task_config):
    prompt_path = task_config['file_path']['prompt_path']
...
def eval_local(args, model, model_aux_list0, model_aux_list1,
    ...
    for obj in tqdm(test_df):
        ...
        if args.task == 'flores':
            ...
            prompt = build_inst_prompt(args.task, args.model, obj["question"], llms_config, task_config)
            prompt_aux_list = []
            for aux_model in args.aux_models:
                prompt_aux = build_inst_prompt(args.task, aux_model, obj["question"], llms_config, task_config)
        ...
        input_ids = enc.input_ids.to(devices[0])              # shape: [1, T]
        attention_mask = enc.attention_mask.to(devices[0])
        ...
        while num_of_new_tokens < args.MAX_NEW_TOKEN:
            ...
            with torch.no_grad():
                ...
                for aux_idx, aux_item in enumerate(zip(tokenizer_aux_list[:len0], model_aux_list0, sparse_matrix_list[:len0])):
                    ...
                    if args.task == 'flores':
                        input_ids_aux = tokenizer_aux(prompt, return_tensors="pt").input_ids.to(device)
                    else:
                        input_ids_aux = tokenizer_aux(prompt_aux_list[aux_idx], return_tensors="pt").input_ids.to(device)
                    ...
                    logits_aux = torch.spmm(sparse_matrix.to(device), logits_aux)
            ...
            if args.aux_method == "linear_sum":
                ...
                res_list = [logits] + logits_aux_list

            elif args.aux_method == "drop":
                tmp_list = [logits] + logits_aux_list
                res_list = []
            ...
            probs = torch.stack([res.flatten() for res in res_list], dim=0)  # shape [num_models, vocab_size]
            token_confs = torch.ones_like(probs)
            model_confs = torch.ones((len(res_list), 1), dtype=torch.float32, device=probs.device) / len(res_list)  # shape [num_models, 1]

            if args.ensemble_method != 'vanilla':
                p_star = probs[0, :]
                if args.ensemble_method[:4] in ['tas2', 'tas3']:
                    p_star = torch.mean(probs, dim=0, keepdim=True)
                token_confs = torch.exp(-torch.abs(probs - p_star))
                if args.ensemble_method[:4] == 'tas3':
                    token_confs[0, :] = 1.0 # exclude main model

                if args.ensemble_method[-4:] == 'mas2':
                    model_confs = model_confs * torch.sum(token_confs, dim=1, keepdim=True)
            
            avg_probs = torch.sum(model_confs * token_confs * probs, dim=0, keepdim=True)
            ensemble_logits = F.normalize(avg_probs, p=1, dim=-1)
...
def main(args, task_config):
    ...
    for id, aux_model in enumerate(args.aux_models):
        ...
        model_aux = AutoModelForCausalLM.from_pretrained(model_aux_ckpt, config=config_aux, torch_dtype=torch.bfloat16, trust_remote_code=True, low_cpu_mem_usage=True).to(devices[(id + 1) % len(devices)])
        if id > 0:
        ...
        sparse_matrix = torch.sparse_coo_tensor(scipy_matrix.nonzero(), scipy_matrix.data, scipy_matrix.shape).t().to(devices[0]).to(torch.float32)#[32000,65024]
    ...
    model = AutoModelForCausalLM.from_pretrained(model_ckpt, config=config, torch_dtype=torch.bfloat16,trust_remote_code=True, low_cpu_mem_usage=True).to(devices[0])
    ...
    if not os.path.exists(os.path.join(args.save_dir, mode_str, args.ensemble_method)):
        os.makedirs(os.path.join(args.save_dir, mode_str, args.ensemble_method))
    ...
    if args.task == 'flores':
        ...
        predictions = eval_local(args, model, model_aux_list0, model_aux_list1, tokenizer, tokenizer_aux_list, sparse_matrix_list, test_df, dev_df, src, tgt, llms_config=llms_config, task_config=task_config)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--aux_models", nargs="+", type=str, help="A list of models", default=None)
    ...
    parser.add_argument("--run_mode", type=str, required=True, choices=["dev", "test"])
    parser.add_argument("--save_dir", type=str, default="/home/qiyu6/EVA/ensemble/results")
    parser.add_argument("--ensemble_method", type=str, default="vanilla", choices=["vanilla", "tas", "tas2", "tas2+mas2", "tas3+mas2"])
    parser.add_argument("--src", type=str, default="zho_simpl")
    parser.add_argument("--tgt", type=str, default="eng")

    args = parser.parse_args()

    if args.aux_models is None:
        args.aux_models = []

    with open("confs/tasks.json", "r", encoding="utf-8") as f:
        tasks_config = json.load(f)
    assert args.task in tasks_config, f"Task '{args.task}' not found in task_config.json"
    task_config = tasks_config[args.task]

    args.save_dir = os.path.join(args.save_dir, args.task, args.run_mode)
    args.MAX_NEW_TOKEN = task_config['run_parameter']['max_new_tokens']
    
    if args.aux_lambda is None:
        if args.aux_method != "drop":
            num_of_aux_models = len(args.aux_models)
            args.aux_lambda = [1 / (num_of_aux_models + 1) for _ in range(num_of_aux_models)]
        else:
            args.aux_lambda = None
    main(args, task_config)
